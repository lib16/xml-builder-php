...
class Xml
{
    ...
    public function setXmlns(string $uri = null, string $prefix = null): self
    {
        if (is_null($uri)) {
            ...
            $this->attributes->set(
                ! empty($prefix) ? 'xmlns:' . $prefix : 'xmlns',
                $uri
            );
        }
        return $this;
    ...
    public function boolAttrib(
        string $name,
        $value,
        string $comparisonAttribute = null
    ): self {
        if (
            ! is_bool($value)
    ...
    {
        $markup = '';
        $hasContent = static::HTML_MODE_ENABLED
            ? $v->content !== null
            : $v->content != '';
        if ($v->hasTags) {
            if ($hasContent) {
                ...
                $markup .= $v->whitespace . $this->openingTag($v) . $v->content . $this->closingTag($v);
            } else {
